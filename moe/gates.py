"""Our implementation of Top-k and MOESART routers

Top-k Ref: Noam Shazeer, Azalia Mirhoseini, Krzysztof Maziarz, Andy Davis, Quoc Le, Geoffrey Hinton, and Jeff Dean. Outrageously Large Neural Networks: The Sparsely-Gated Mixture-of-Experts Layer. In ICLR 2017.
The implementation is based on the description in Section 2.1.

MOESART Ref: Shibal Ibrahim, Wenyu Chen, Hussein Hazimeh, Natalia Ponomareva, and Rahul Mazumder. MOESART: An Effective Sampling-based Router for Sparse Mixture of Experts. In ICML 2024.
...
#        f = torch.cat(f, dim=2)
        ...
        topk_scattered = (-torch.inf*torch.ones_like(gate_logits)).scatter_(1, topk.indices, topk.values)
...
#        print("Sparsity", metrics['avg_sparsity'])
...
#        return y, soft_averages, hard_averages, s_concat, self.loss # reg loss
#        return y, self.loss # reg loss
        return g_permuted, self.loss # reg loss
...
"""A PyTorch layer for MOESART router."""    
class MOESARTGate(nn.Module):
    """
    ...
    def __init__(self, config, task=0):
        super(MOESARTGate, self).__init__()
        self.task = 0
        self.nb_experts = config["nb_experts"]
        self.k = int(config["k"])
        self.jitter = config["jitter"] if config["jitter"] is not None else False
        self.epsilon = torch.tensor(1e-6)
        self.tau = torch.tensor(config["tau"])
        self.trimmed_lasso_reg = torch.tensor(config["trimmed_lasso_reg"])
        ...
        if not deterministic:
            if biasness == "zero":
                adjusted_gate_logits -= torch.log(
                    torch.clamp(torch.tensor(k, dtype=gate_logits.dtype) * (g + 1e-10), min=1e-10)
                )
                ...
                adjusted_gate_logits -= torch.where(
                    ...
                    torch.zeros_like(g),
                    torch.log(torch.clamp(torch.tensor(k - 1, dtype=gate_logits.dtype) * (g + 1e-10), min=1e-10)),
                )
            elif biasness == "large":
                pass
            else:
