...
class TranslationMoEConfig(TranslationConfig):
    ...
    route_method: str = field(
        default="topk",
        metadata={"help": "which routing method to use; use topk or moesart"},
    )
    k: int = field(
        default=2,
        metadata={
            "help": "number of experts used per sample"
        },
    )
    trimmed_lasso_reg: float = field(
        default=0.0,
        metadata={"help": "if > 0.0, it penalizes smallest n-k entries in router scores."},
    )
...
class TranslationMoETask(TranslationTask):
    ...
    def _get_loss(self, sample, model, criterion, expert_num=None):
        ...
        gate_losses = encoder_out.get("gate_losses", 0.0)+net_output[1].get("gate_losses", 0.0)
...
#             "selection": net_output[1].get("selection", None),
            "gate_losses": gate_losses.data,
        }

        return loss, gate_losses, logits, sample_size, logging_output
...
#                 expert_num = None
#                 loss1, trimmed_lasso_loss1, logits1, sample_size, logging_output1 = self._get_loss(sample, model, criterion, expert_num)
                loss1, gate_losses1, logits1, sample_size, logging_output1 = self._get_loss(sample, model, criterion)
...
#         loss = loss1 + loss2 + consistency_loss * self.cfg.consistency_alpha
        loss = loss1 + gate_losses1.to(loss1.get_device()) * self.cfg.trimmed_lasso_reg

        logging_output = {
#             "loss": torch.tensor([logging_output1["loss"], logging_output2["loss"]]),
            "loss": torch.tensor([logging_output1["loss"]]),
#             "nll_loss": torch.tensor([logging_output1["nll_loss"], logging_output2["nll_loss"]]),
            "nll_loss": torch.tensor([logging_output1["nll_loss"]]),
            "ntokens": sample["ntokens"],
            "nsentences": sample["target"].size(0),
            "sample_size": sample_size,
#             "consistency": consistency_loss.data,
            "gate_losses": gate_losses1.data,
        }
...
#         loss_sum = sum(log.get("consistency", 0) for log in logging_outputs)
        gate_losses_mean = sum(log.get("gate_losses", 0) for log in logging_outputs)
        sample_size = sum(log.get("sample_size", 0) for log in logging_outputs)
#         metrics.log_scalar(
#             "consistency", loss_sum / sample_size / math.log(2), sample_size, round=3
#         )
        metrics.log_scalar(
            "gate_losses", gate_losses_mean, sample_size, round=3
        )
